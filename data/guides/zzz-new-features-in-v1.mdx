...
You can map mdx news content to layout components by configuring the frontmatter field. For example, this post is written with the new `PostSimple` layout!
...
### Configuring a news post frontmatter
...
tags: ['next-js', 'tailwind', 'guide']
draft: false
summary: 'Introducing the new layout features - you can map mdx news content to layout components by configuring the frontmatter field'
layout: PostSimple
---
```

You can configure the default layout in the respective page section by modifying the `DEFAULT_LAYOUT` variable.
The `DEFAULT_LAYOUT` for news posts page is set to `PostLayout`.
...
### Multiple authors in news post

The frontmatter of a news post accepts an optional `authors` array field. If no field is specified, it is assumed that the default author is used. Simply pass in an array of authors to render multiple authors associated with a post.
...
A demo of a multiple authors post is shown in [Introducing Tailwind Nextjs Starter News post](/news/introducing-tailwind-nextjs-starter-news).
...
Two components are exported, a default `NewsletterForm` and a `NewsNewsletterForm` component, which is also passed in as an MDX component
and can be used in a news post:

```jsx
<NewsNewsletterForm title="Like what you are reading?" />
```
...
the component changes you are interested in to your existing news rather than migrating everything over.

Nonetheless, if you want to do so and have not changed much of the template, you could clone the new version and copy over the news post over to the new template.

Another alternative would be to pull the latest template version with the following code:

```bash
git remote add template git@github.com:timlrx/tailwind-nextjs-starter-news.git
git pull template v1 --allow-unrelated-histories
rm -rf node_modules
```

You can see an example of such a migration in this [commit](https://github.com/timlrx/timlrx.com/commit/bba1c185384fd6d5cdaac15abf802fdcff027286) for my personal news.
